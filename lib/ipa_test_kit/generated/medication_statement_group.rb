...
module IpaTestKit
  class MedicationStatementGroup < Inferno::TestGroup
    ...
    short_description 'Verify support for the server capabilities required by the IPA MedicationStatement Profile.'
    description %(
# Background
...
must contain resources conforming to the IPA MedicationStatement Profile as
...
the [IPA MedicationStatement Profile](http://hl7.org/fhir/uv/ipa/StructureDefinition/ipa-medicationstatement). Each element is checked against
...
fail if any attempted read fails.
    ...
    id :ipa_010_medication_statement
    run_as_group

    def self.metadata
      @metadata ||= Generator::GroupMetadata.new(YAML.load_file(File.join(__dir__, 'medication_statement', 'metadata.yml')))
    end
...
#    test from: :ipa_010_medication_statement_provenance_revinclude_search_test
    ...
    test from: :ipa_010_medication_statement_reference_resolution_test
  end
end
