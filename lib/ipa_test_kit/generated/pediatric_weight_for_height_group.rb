...
module IpaTestKit
  class PediatricWeightForHeightGroup < Inferno::TestGroup
    ...
    short_description 'Verify support for the server capabilities required by the IPA Pediatric Weight for Height Observation Profile.'
    description %(
# Background

The IPA Pediatric Weight for Height Observation sequence verifies that the system under test is
able to provide correct responses for Observation queries. These queries
must contain resources conforming to the IPA Pediatric Weight for Height Observation Profile as
...
the [IPA Pediatric Weight for Height Observation Profile](http://hl7.org/fhir/uv/ipa/StructureDefinition/pediatric-weight-for-height). Each element is checked against
...
fail if any attempted read fails.
    ...
    id :ipa_010_pediatric_weight_for_height
    run_as_group
    ...
    test from: :ipa_010_pediatric_weight_for_height_patient_code_search_test
    test from: :ipa_010_pediatric_weight_for_height_patient_category_date_search_test
    test from: :ipa_010_pediatric_weight_for_height_patient_category_status_search_test
    test from: :ipa_010_pediatric_weight_for_height_patient_code_date_search_test
    test from: :ipa_010_pediatric_weight_for_height_patient_category_search_test
    test from: :ipa_010_pediatric_weight_for_height_read_test
    test from: :ipa_010_pediatric_weight_for_height_provenance_revinclude_search_test
    test from: :ipa_010_pediatric_weight_for_height_validation_test
    test from: :ipa_010_pediatric_weight_for_height_must_support_test
    test from: :ipa_010_pediatric_weight_for_height_reference_resolution_test
  end
end
