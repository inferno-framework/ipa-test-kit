...
module IpaTestKit
  class OrganizationGroup < Inferno::TestGroup
    ...
    short_description 'Verify support for the server capabilities required by the IPA Organization Profile.'
    description %(
# Background

The IPA Organization sequence verifies that the system under test is
able to provide correct responses for Organization queries. These queries
must contain resources conforming to the IPA Organization Profile as
...
the [IPA Organization Profile](http://hl7.org/fhir/uv/ipa/StructureDefinition/ipa-organization). Each element is checked against
...
fail if any attempted read fails.
    ...
    test from: :ipa_010_organization_read_test
    test from: :ipa_010_organization_name_search_test
    test from: :ipa_010_organization_address_search_test
    test from: :ipa_010_organization_validation_test
    test from: :ipa_010_organization_must_support_test
    test from: :ipa_010_organization_reference_resolution_test
  end
end
