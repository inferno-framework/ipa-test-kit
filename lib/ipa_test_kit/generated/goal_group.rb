...
module IpaTestKit
  class GoalGroup < Inferno::TestGroup
    title 'Goal Tests'
    short_description 'Verify support for the server capabilities required by the IPA Goal Profile.'
    description %(
# Background

The IPA Goal sequence verifies that the system under test is
able to provide correct responses for Goal queries. These queries
must contain resources conforming to the IPA Goal Profile as
...
the [IPA Goal Profile](http://hl7.org/fhir/uv/ipa/StructureDefinition/ipa-goal). Each element is checked against
...
fail if any attempted read fails.
    ...
    test from: :ipa_010_goal_patient_search_test
    test from: :ipa_010_goal_patient_lifecycle_status_search_test
    test from: :ipa_010_goal_patient_target_date_search_test
    test from: :ipa_010_goal_read_test
    test from: :ipa_010_goal_provenance_revinclude_search_test
    test from: :ipa_010_goal_validation_test
    test from: :ipa_010_goal_must_support_test
    test from: :ipa_010_goal_reference_resolution_test
  end
end
